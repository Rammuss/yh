...
$query_check = "SELECT COUNT(*) FROM compras WHERE numero_factura = $1";
$result_check = pg_query_params($conn, $query_check, [$numero_factura]);
$count = pg_fetch_result($result_check, 0, 0);

if ($count > 0) {
    // Número de factura ya existe
    header('Location: registrar_compras.html?error=numero_factura_existe');
    exit();
}
...
try {
    ...
    header('Location: registrar_compras.html?success=factura_registrada');
    exit();
} catch (Exception $e) {
    // Revertir cambios en caso de error
    pg_query($conn, "ROLLBACK");
    header('Location: registrar_compras.html?error=insertar_factura');
    exit();
}
