        ...
        <div class="container">
            ...
            <div class="box">
                ...
                <form id="formVentaCabecera">
                    ...
                    <div class="field">
                        <label class="label" for="nota_credito_id">Número de Nota de Crédito(Opcional):</label>
                        <div class="control">
                    ...
                    <div class="container">
                        <div class="field"> <label class="label">Seleccionar Solicitud de servico (Opcional)</label>
                            <div class="control">
                                <div class="select is-fullwidth"> <select id="solicitudSelect" onchange="actualizarDatos()"> <!-- Opciones generadas dinámicamente en tu código PHP -->
                                        <option value="" data-total="">Seleccione una solicitud</option>
                                    </select> </div>
                            </div>
                        </div>
                        <!-- Inputs ocultos para almacenar el ID de solicitud y el monto total -->
                        <input type="hidden" id="solicitudId" name="solicitud_id">
                        <input type="hidden" id="montoTotal" name="monto_total">
    ...
    <script>
        function obtenerDatosFormulario() {
            ...
            const cabecera = {
                ...
                solicitud_id: document.getElementById('solicitudId')?.value || "", // Capturar el valor del campo solicitud_id
                monto_total: document.getElementById('montoTotal')?.value || "" // Capturar el valor del campo monto_total
            };
    ...
    <script>
        function actualizarDatos() {
            var select = document.getElementById('solicitudSelect');
            var solicitudId = select.options[select.selectedIndex].value;
            var montoTotal = select.options[select.selectedIndex].getAttribute('data-total');
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            fetch('../venta_v2/obtener_solicitudes_servicios.php').then(response => response.json()).then(data => {
                const select = document.getElementById('solicitudSelect');
                data.forEach(solicitud => {
                    const option = document.createElement('option');
                    option.value = solicitud.id_cabecera;
                    option.setAttribute('data-total', solicitud.monto_total);
                    option.textContent = `${solicitud.cliente_nombre} (RUC: ${solicitud.ruc_ci})`;
