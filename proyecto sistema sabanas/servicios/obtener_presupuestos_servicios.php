<?php
    ...
    try {
        ...
        $query = "SELECT pc.id_cabecera AS id,
                         c.nombre AS cliente,
                         c.ruc_ci AS ci,
                         pc.fecha,
                         pc.monto_total,
                         pc.descuento,
                         pc.estado
                  FROM presupuesto_cabecera_servicio pc
        ...
        if ($fecha) {
            $query .= " AND pc.fecha = $" . $paramIndex++ . "::date"; // Casting explícito a DATE
            $params[] = $fecha;
        }
        if ($estado) {
            $query .= " AND pc.estado = $" . $paramIndex++ . "::varchar"; // Casting explícito a VARCHAR
            $params[] = $estado;
        }
        if ($ci) {
